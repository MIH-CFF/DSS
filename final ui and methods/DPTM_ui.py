...
class Works(QThread):
    ...
    def __init__(self, fname, k_len, thresh,method):
        super().__init__()
        self.fname = fname
        self.k_len = k_len
        self.thresh = thresh
    ...
    def run(self):
        try:
            ...
            result,text = dtm.read_fas1(self.fname, self.k_len, self.thresh,self.method)
...
class PhyloTreeWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Dynamic Part-wise Template Matching Method")
        self.setWindowIcon(QIcon(resource_path("images\\demo_logo.png")))
        self.resize(900, 600)
        self.move(20,50)
        ...
        opts_m = QHBoxLayout()
        opts_m.addStretch()
        opts_m_lbl=QLabel("Select Method : ")
        opts_m_lbl.setStyleSheet("border:none;""background-color:transparent")
        opts_m.addWidget(opts_m_lbl)
        self.t_df_upgma = QRadioButton("UPGMA")
        self.t_df_upgma.setChecked(True)
        self.t_cm_nj = QRadioButton("NJ")
        self.button_m=QButtonGroup(self)
        self.button_m.addButton( self.t_df_upgma)
        self.button_m.addButton(self.t_cm_nj)
        opts_m.addWidget(self.t_df_upgma)
        opts_m.addWidget(self.t_cm_nj)
        layout.addLayout(opts_m)
    ...
    def draw_phylo(self):
        ...
        thresh = self.t_spin.value() if self.t_cm_custom.isChecked() else 50
        method='upgma' if self.t_df_upgma.isChecked() else 'nj'
                
        self.loading.show() 
        self.loading.show()

        self.work = Works(self.fname, k_len, thresh,method)
    ...
    def handle_result(self, tree_rcv,text):
        ...
        try:
            ...
            lbl_pix = lbl_pix.scaled(700, 400, Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
