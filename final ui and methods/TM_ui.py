...
class Works(QThread):
    ...
    def __init__(self, fname, partition, method):
        super().__init__()
        self.fname = fname 
    ...
    def run(self):
        try:
            tm=TM_func.TM()
            result,text = tm.run(directory=self.fname,partition=self.partition,method=self.method)
...
class PhyloTreeWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Template Matching Method")
        self.setWindowIcon(QIcon(resource_path("images\\demo_logo.png")))
        self.resize(900, 600)
        self.move(20,50)
    ...
    def draw_phylo(self):
        ...
        self.work = Works(self.fname,partition, method)
