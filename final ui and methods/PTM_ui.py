...
class Works(QThread):
    ...
    def __init__(self, fname, partition,base_length, method):
        super().__init__()
        self.fname = fname
    ...
    def run(self):
        try:
            ptm=PTM_func.PTM()
            result,text = ptm.run(directory=self.fname,partition=self.partition,baseLength=self.base_length,method=self.method)
...
class PhyloTreeWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Part-wise Template Matching Method")
        self.setWindowIcon(QIcon(resource_path("images\\demo_logo.png")))
        self.resize(900, 600)
        self.move(100,50) 
    ...
    def draw_phylo(self):
        if self.fname==None:
            ...
            return
        partition = self.spin_k.value() if self.rb_custom.isChecked() else 1
        base_length = self.spin_b.value() if self.rb_custom.isChecked() else 4
        method='upgma' if self.t_df_upgma.isChecked() else 'nj'
                
        self.loading.show() 
        self.loading.show()

        self.work = Works(self.fname,partition,base_length, method)
